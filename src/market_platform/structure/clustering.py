...
def create_price_zone(candidates: Sequence[PriceLevelCandidate]) -> PriceZone:
    ...
    Candidates are sorted by observed_at, kind, and price before zone boundaries
    are derived. Source methods are deduplicated in first-seen order from the
    sorted candidates.
    """
...
def _candidate_sort_key(candidate: PriceLevelCandidate) -> tuple[str, str, float, str]:
    return (
        candidate.observed_at.isoformat(),
        candidate.kind.value,
        candidate.price,
